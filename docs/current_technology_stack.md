...
6. Zepto MCP provider adapter for cart sync.
...
- `grocery_cart_items`: shared native grocery cart.
...
- Kitch syncs native cart rows into Zepto only after user intent.
...
- If the same fridge upload includes a grocery request, a follow-up grocery-planning turn runs after pantry updates.
