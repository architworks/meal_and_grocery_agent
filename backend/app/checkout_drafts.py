...
def draft_row_to_review(row: Dict[str, Any] | None) -> Dict[str, Any] | None:
    ...
    return {
        ...
        "provider": row.get("provider") or "zepto",
        ...
        "zepto_cart": row.get("provider_cart"),
...
def _order_blockers(
    ...
    if result.get("status") != "success":
        blockers.append("Zepto did not confirm every selected cart item as orderable.")
    if len(matched_items) != len(native_items) or unavailable_items:
        blockers.append("Every selected native item must have a confirmed orderable Zepto product.")
    if checkout_context.get("address_error"):
        blockers.append("Zepto address options could not be read.")
    if checkout_context.get("payment_error"):
        blockers.append("Zepto payment options could not be read.")
    if (result.get("store_context") or {}).get("status") != "ready":
        blockers.append("Zepto store context is not ready for the selected address.")
...
def save_initial_draft(
    ...
    blockers = _order_blockers(native_items, result)
    payload: Dict[str, Any] = {
        ...
        "provider_cart": result.get("zepto_cart"),
    ...
    return draft_row_to_review(save_provider_checkout_draft(payload)) or {}
...
def save_revalidated_draft(
    ...
    payload["snapshot_hash"] = checkout_snapshot_hash(payload)
    return draft_row_to_review(save_provider_checkout_draft(payload)) or {}, changed
...
def invalidate_draft_for_native_drift(
    ...
    blockers.append("The native grocery selection changed after the Zepto cart was reviewed.")
    payload = {
        "selected_payment_method_id": None,
        "order_review_acknowledged": False,
